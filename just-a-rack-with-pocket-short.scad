/**
 * 10-Inch Unibody Server Rack (6U) - "Just a Rack" with USW Flex Mini Pocket
 * Prints in one piece on Bambu Labs H2D - front face down, no supports
 *
 * Includes a pocket on the right interior wall for a Ubiquiti USW Flex Mini
 * switch, with 45-degree ramp underneath for support-free printing.
 *
...
// =========================================
// USW FLEX MINI POCKET (right wall, bottom)
// =========================================
...
// Position: against right wall inner surface, flush with bottom, centered Z
...
module usw_pocket_cavity() {
    ...
    // Limited to the 3mm pocket wall so the rack outer wall stays solid
    hull() {
 ...
 *   - Right wall: top 2 rows only (Y3, Y4) - bottom rows replaced by pocket
...
module rack_towers() {
    ...
    // --- Right wall: top 2 rows only (skip bottom 2 for pocket) ---
    for (iy = [2:3]) {
        for (iz = [0:2]) {
    ...
    pocket_tower_z_front = pocket_z_mm - pocket_tower_spacing;
...
/**
 * Complete Rack Assembly with USW Flex Mini Pocket
 *
 * BOUNDING BOX: [0, 0, 0] to [264, 310, 200]
 */
module rack() {
    union() {
        difference() {
